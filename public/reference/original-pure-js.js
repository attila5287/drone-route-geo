...
$geoInputs.forEach((inputEl) => inputEl.addEventListener("change", handlerGeo));
...
let fetchData = () =>
  draw.getAll().features.length ? draw.getAll() : testpoly;
...
function updateArea(e) {
  ...
  if (polygon.features.length > 0) {
    ...
    map
      .getSource("line-src")
      .setData(geometricRoute(polygon, fetchUserInput()));
